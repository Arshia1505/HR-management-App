...
const MAX_RETRIES = Number(process.env.EMAIL_MAX_RETRIES || 4);
const BASE_RETRY_DELAY_MS = Number(process.env.EMAIL_RETRY_BASE_MS || 800);
const PER_RECIPIENT_DELAY_MS = Number(process.env.EMAIL_SEND_DELAY_MS || 250);
const MAX_CONNECTIONS = Number(process.env.EMAIL_MAX_CONNECTIONS || 5);
...
async function sendWithRetries(transporter, mailOpts, to) {
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const errMsg = err?.message || String(err);
      console.error(`send attempt ${attempt} for ${to} failed:`, errMsg);
      if (attempt === MAX_RETRIES) return { to, success: false, error: errMsg };
      const backoff = BASE_RETRY_DELAY_MS * Math.pow(2, attempt - 1);
      await wait(backoff);
    }
  }
...
export async function POST(req) {
  try {
    ...
    // Accept either `emails` (array) or `memberIds` (array)
    let { emails, memberIds, subject, message } = body || {};
    subject = subject ?? '(No subject)';
    message = message ?? '';

    // if memberIds provided -> resolve to emails by calling get-members API
    if ((!emails || !Array.isArray(emails) || emails.length === 0) && Array.isArray(memberIds) && memberIds.length > 0) {
      // require APP_URL env (fallback to localhost)
      const base = process.env.APP_URL?.replace(/\/$/, '') || 'http://localhost:3000';
      try {
        const res = await fetch(`${base}/api/get-members`);
        if (!res.ok) throw new Error('Failed to fetch members for resolution');
        const data = await res.json();
        const allMembers = data.members || [];
        const idSet = new Set(memberIds.map(id => String(id)));
        emails = allMembers
          .filter(m => idSet.has(String(m._id ?? m.id)))
          .map(m => m.email)
          .filter(Boolean);
      } catch (err) {
        console.error('Error resolving memberIds to emails:', err);
        return new Response(JSON.stringify({ success: false, error: 'Failed to resolve member IDs' }), { status: 500 });
      }
    }

    if (!emails || !Array.isArray(emails) || emails.length === 0) {
      return new Response(JSON.stringify({ success: false, error: 'No recipient emails provided' }), { status: 400 });
    }

    // normalize emails (trim + dedupe)
    ...
    if (!SMTP_HOST || !SMTP_USER || !SMTP_PASS || !FROM_EMAIL) {
      console.error('Missing SMTP env variables');
      return new Response(JSON.stringify({ success: false, error: 'SMTP not configured on server' }), { status: 500 });
    }

    // create transporter (pooling)
    const transporter = nodemailer.createTransport({
      host: SMTP_HOST,
      port: Number(SMTP_PORT || 587),
      secure: Number(SMTP_PORT) === 465,
